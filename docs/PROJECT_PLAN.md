...
Initial input should support one word or a pasted list, preferably one entry per line. CSV import is a proposed convenience. Preview the parsed list before saving.
...
Implemented first slice on 2026-09-06: short rounds draw up to five selected meanings from Meine Auswahl after vocabulary has been saved. Previously learned due items take priority, with new words filling remaining places. No set selection is required. Hide the vocabulary workspace during a round so it does not reveal answers; Pause restores it.
...
New or previously failed meanings start with a learning card, followed by multiple choice and typed recall. Distractors come only from saved words with different definitions; with no distinct distractor, use a self-assessed flashcard. Known due meanings use typed recall. Each word also receives one due grammar dimension per round, rotating through available forms. Missing grammar is skipped; plural-only words have no singular-article/plural-conversion quiz. Grammar prompts include the saved definition to distinguish senses.
...
Show the saved answer after grading. The first mistake for a dimension inserts one typed retry later in the same round. Schedule each meaning/grammar dimension independently: successful due reviews advance through 1, 3, 7, 14, 30 and 60 elapsed days; failure resets to a 10-minute review. Aggregate by round and question key: any mistake in that round prevents promotion, even after a correct retry; several correct answers in one round promote only once. Correct early practice does not move the due date. “Auch vorzeitig üben” allows optional early rounds. Times use UTC milliseconds, so day boundaries do not depend on local midnight.
...
- Includes a possible API issue warning for encoded words containing umlauts, ß, or spaces. Treat this as a test case, not a confirmed current API defect.
...
| 2026-09-06 | User chose a manual Wiktionary fallback for failed lookups such as Übungen. Added external search plus a paste/edit form, noun grammar, optional example/source, and session-selection export. Automatic Unicode lookup recovery remains unresolved. |
