...
- Local verification passed: all 80 automated tests, JavaScript syntax checks, the clean static build, root HTTP smoke test, old-route 404 check, and `git diff --check`. A browser pass confirmed the four root navigation items, version-free page title/header, and the single **App-Version 2** marker in Settings. The build now clears `dist/` first so removed routes cannot survive as stale output. GitHub Pages publication is still in progress; record the exact deployment result here before calling the release complete.
