...
- For each queued word, select one or more dictionary meanings or add a manual meaning, then choose **Fertig, nächstes Wort** or **Überspringen**. You can reopen any word from the queue. Pasted/file queues are held only in the open page; selected meanings are saved through the usual **In Google Sheets speichern** button in Meine Auswahl.
- The connected vocabulary spreadsheet can also contain **import_baselist** with `german_word` and `english_meaning` columns. Opening the vocabulary table reads its still-open rows; **Offene Wörter aus Google Sheets laden** refreshes and starts them. On the first processed row, the app adds a third `status` header. **Überspringen** writes `skipped` immediately. Selecting or manually creating a meaning queues that row for `added`; **Fertig, nächstes Wort** only advances through the queue and is not required when saving early. After **In Google Sheets speichern** confirms the vocabulary meaning, the source row is written as `added`. Rows already marked `skipped` or `added` are not offered again. Do not reorder or edit active baselist rows while reviewing them, because the app verifies both the row number and its original word/meaning before writing a status.
